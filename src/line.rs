...
impl Line {
    pub fn new(pos: Vec3, length: usize, axis: Axis, color: Color, on: bool) -> Line {
        let mut segments = Vec::new();
        let mid_pos = match &axis {
            Axis::X => Vec3 { x: pos[0] + (length) as f32 / 2.0, y: pos[1], z: pos[2] },
            Axis::Y => Vec3 { x: pos[0], y: pos[1] + (length) as f32 / 2.0, z: pos[2] },
            Axis::Z => Vec3 { x: pos[0], y: pos[1], z: pos[2] + (length) as f32 / 2.0 },
        };
